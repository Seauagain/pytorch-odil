...
gamma = torch.tensor(1.0, dtype=dtype) * (-1.) * gamma_ch
...
def gauss_sol3d_tf(x, y, z, dx, dy, dz, init_scale):
    ...
    # Gaussian function calculation
    gauss = M / torch.pow(4 * torch.tensor(np.pi, dtype=x.dtype) * Dt, 3/2) * torch.exp(- (torch.pow(x_scaled, 2) + torch.pow(y_scaled, 2) + torch.pow(z_scaled, 2)) / (4 * Dt))

    # Apply thresholds
    gauss = torch.where(gauss > 0.1, gauss, torch.zeros_like(gauss))
    gauss = torch.where(gauss > 1, torch.ones_like(gauss, dtype=dtype), gauss)
...
def unet_loss(unet_data, c_field, th_low):
    # Convert unet_data numpy array to a PyTorch tensor
    unet_data_tensor = torch.tensor(unet_data, dtype=DTYPE)
    
    # Ensure c_field is a float tensor (in case it isn't)
    c_field = c_field.float()
    
    # Create a mask where unet_data is 1
    mask = torch.eq(unet_data_tensor, 1.0)
    
    # Calculate the part of c_field that is under the threshold where unet_data is 1
    under_threshold = torch.lt(c_field, th_low)
    relevant_under_threshold = torch.logical_and(mask, under_threshold)
    
    # Calculate losses where the condition is met
    # Loss is linearly scaled with c_field, reaching maximum when c_field is 0
    losses = torch.where(relevant_under_threshold, (th_low - c_field) / th_low, torch.tensor(0.0, dtype=torch.float32))
    
    # Calculate the maximum possible loss
    max_possible_loss = torch.where(mask, torch.tensor(1.0, dtype=torch.float32), torch.tensor(0.0, dtype=torch.float32))
    
    # Compute the total loss and normalize by the maximum possible loss
    total_loss = torch.sum(losses)
    max_loss = torch.sum(max_possible_loss)
    
    # Avoid division by zero
    if max_loss == 0:
        normalized_loss = torch.tensor(0.0, dtype=torch.float32)
    else:
        normalized_loss = total_loss / max_loss
...
def transform_c(c, mod):
    ...
    if mod == np:
        ...
        c_masked = torch.cat([c_masked[:0], torch.zeros_like(c_masked[:1]), c_masked[1:]], dim=0)
...
def transform_txyz(tx, ty, tz, x, y, z, mod):
    ...
    if mod == np:
        ...
        # Fix the trajectories for the initial time step.
        tx = torch.concat([tx[:nth], x[nth:nth+1], tx[nth + 1:]], dim=0)
        ty =  torch.concat([ty[:nth], y[nth:nth+1], ty[nth + 1:]], dim=0)
        tz =  torch.concat([tz[:nth], z[nth:nth+1], tz[nth + 1:]], dim=0)

        # Fix the spatial boundary particles in all three dimensions.
        tx =  torch.concat([x[:, :1, :, :], tx[:, 1:-1, :, :], x[:, -1:, :, :]], dim=1)
        ty =  torch.concat([y[:, :, :1, :], ty[:, :, 1:-1, :], y[:, :, -1:, :]], dim=2)
        tz =  torch.concat([z[:, :, :, :1], tz[:, :, :, 1:-1], z[:, :, :, -1:]], dim=3)

        # Create tensors of the initial positions for particles outside the skull in all three dimensions.
        # print(type(outside_skull_mask))
        x_masked = x * outside_skull_mask
        y_masked = y * outside_skull_mask
        z_masked = z * outside_skull_mask
...
def compute_displacement(tx, ty, tz):
    ...
    initial_position_x = initial_position_x.unsqueeze(0)
    initial_position_y = initial_position_y.unsqueeze(0)
    initial_position_z = initial_position_z.unsqueeze(0)
...
def gradient(tensor, step, axis, final_op=False):
    ...
    tensor_before = torch.roll(tensor, shifts=(1,), dims=(axis,))
    tensor_after = torch.roll(tensor, shifts=(-1,), dims=(axis,))
    ...
    if axis == 1:
        ...
        gradient = torch.concat([gradient_left[:, None, :, :], gradient[:, 1:-1, :, :], gradient_right[:, None, :, :]], dim=axis)
        ...
        gradient = torch.concat([gradient_top[:, :, None, :], gradient[:, :, 1:-1, :], gradient_bottom[:, :, None, :]], dim=axis)
        ...
        gradient = torch.concat([gradient_front[:, :, :, None], gradient[:, :, :, 1:-1], gradient_back[:, :, :, None]], dim=axis)
...
def get_core_loss_tf(c, th_up, segm):
    ...
    # Compute the core loss where core_mask is True, else set to 0
    core_loss = torch.where(core_mask,
                           torch.clamp(th_up - c, min=0, max=torch.finfo(torch.float32).max),
                           torch.zeros_like(core_mask, dtype=torch.float32))
...
def get_edema_loss_tf(c, th_down, th_up, segm):
    ...
    within_range_condition = (c >= th_down) & (c <= th_up) # be careful!
    # Combine the mask with the condition
    final_mask = edema_mask & ~within_range_condition
    ...
    edema_loss = torch.where(final_mask,
                            torch.abs(c - th_down) + torch.abs(c - th_up),
                            torch.zeros_like(final_mask, dtype=torch.float32))
...
def get_outside_segm_loss_tf(c, th_down, segm):
    ...
    final_mask = outside_segm_mask & not_below_condition
    ...
    #                              c - th_down,
    #                              tf.zeros_like(final_mask, dtype=tf.float32))
    outside_segm_loss = torch.where(final_mask,
                                   c - th_down,
                                   torch.zeros_like(final_mask, dtype=torch.float32))
...
def compute_strain_tensor_lagrangian_full(u_x, u_y, u_z, domain):
    ...
    E_xy = 0.5 * (u_x_y + u_y_x + u_x_x*u_x_y + u_y_x*u_y_y)
    E_xz = 0.5 * (u_x_z + u_z_x + u_x_x*u_x_z + u_z_x*u_z_z)
    E_yz = 0.5 * (u_y_z + u_z_y + u_y_x*u_y_z + u_z_y*u_z_z) # E_yz shape:  (128, 72, 72, 72)
...
def m_Tildas(WM, GM, th):
    ...
    WM_tilda_x = torch.where((torch.roll(WM, -1, dims=0) + torch.roll(GM, -1, dims=0) >= th) & (WM + GM >= th), (torch.roll(WM, -1, dims=0) + WM) / 2, torch.tensor(0.0, device=WM.device))
    WM_tilda_y = torch.where((torch.roll(WM, -1, dims=1) + torch.roll(GM, -1, dims=1) >= th) & (WM + GM >= th), (torch.roll(WM, -1, dims=1) + WM) / 2, torch.tensor(0.0, device=WM.device))
    WM_tilda_z = torch.where((torch.roll(WM, -1, dims=2) + torch.roll(GM, -1, dims=2) >= th) & (WM + GM >= th), (torch.roll(WM, -1, dims=2) + WM) / 2, torch.tensor(0.0, device=WM.device))

    GM_tilda_x = torch.where((torch.roll(WM, -1, dims=0) + torch.roll(GM, -1, dims=0) >= th) & (WM + GM >= th), (torch.roll(GM, -1, dims=0) + GM) / 2, torch.tensor(0.0, device=GM.device))
...
def get_D(WM, GM, th, Dw, Dw_ratio):
    ...
    D_plus_x = Dw*( torch.roll( M["WM_t_x"], 1, dims=0) + torch.roll( M["GM_t_x"], 1, dims=0 ) / Dw_ratio)
    D_plus_y = Dw*( torch.roll( M["WM_t_y"], 1, dims=1) + torch.roll( M["GM_t_y"], 1, dims=1 ) / Dw_ratio)
    D_plus_z = Dw*( torch.roll( M["WM_t_z"], 1, dims=2) + torch.roll( M["GM_t_z"], 1, dims=2 ) / Dw_ratio)
...
def operator_adv(ctx):
    ...
    def laplace_roll(q):
        ...
        qxm = torch.roll(q_padded, shifts=(1,), dims=(1,) )
        qxp = torch.roll(q_padded, shifts=(-1,), dims=(1,) )
        qym = torch.roll(q_padded, shifts=(1,), dims=(2,) )
        qyp = torch.roll(q_padded, shifts=(-1,), dims=(2,) )
        qzm = torch.roll(q_padded, shifts=(1,), dims=(3,) )
        qzp = torch.roll(q_padded, shifts=(-1), dims=(3,) )
    ...
    ty = single_var('y')
    tz = single_var('z')
    tx, ty, tz = transform_txyz(tx, ty, tz, x, y, z, torch)
    
    # Tumor
    c = single_var('c')
    c = transform_c(c, mod=torch)
    ...
    tix = torch.floor(dtx).to(torch.int32).clamp(-1, nx - 1)
    tiy = torch.floor(dty).to(torch.int32).clamp(-1, ny - 1)
    tiz = torch.floor(dtz).to(torch.int32).clamp(-1, nz - 1)
    tixp = tix + 1
    tiyp = tiy + 1
    tizp = tiz + 1
    ...
    sx1 = (dtx - tix.to(dtx.dtype)).clamp(0, 1)
    sy1 = (dty - tiy.to(dty.dtype)).clamp(0, 1)
    sz1 = (dtz - tiz.to(dtz.dtype)).clamp(0, 1)
    sx0 = 1 - sx1
    sy0 = 1 - sy1
    sz0 = 1 - sz1
    ...
    th_down = torch.clamp(coeff[5], min=0.20, max=0.35)
    th_up   = torch.clamp(coeff[6], min=0.50, max=0.85)
    ...
    # combined_matter_mask_4d = combined_matter_mask.unsqueeze(0).repeat(10, 1, 1) # be careful # this line is wrong :SmallVector unable to grow. Requested capacity (18446744073709551615) is larger than maximum value 
    # combined_matter_mask_4d = combined_matter_mask.unsqueeze(0).expand(nt, -1, -1)
    combined_matter_mask_4d = combined_matter_mask.expand(nt, -1, -1, -1) ## [128, 72, 72, 72]
    # print("combined_matter_mask_4d: ", combined_matter_mask_4d.shape, combined_matter_mask_4d)
    # exit(0)
    ...
    res.append(torch.clamp(0.11 - D_scalar, min=0, max=100) * params_w)
    res.append(torch.clamp(0.02 - rho, min=0, max=100) * params_w)
    res.append(torch.clamp(1.5 + gamma, min=0, max=100) * params_w)
    ...
    txm = torch.roll(tx, shifts=(1,), dims=(0,))
    tym = torch.roll(ty, shifts=(1,), dims=(0,))
    tzm = torch.roll(tz, shifts=(1,), dims=(0,))

    txp = torch.roll(tx, shifts=(-1,), dims=(0,))
    typ = torch.roll(ty, shifts=(-1,), dims=(0,))
    tzp = torch.roll(tz, shifts=(-1,), dims=(0,))
...
def calculate_symmetry_loss(healthy, scale_factor=1):
    ...
    if scale_factor > 1:
        # Reshape the tensor to 5D for pooling (batch_size, channels, depth, height, width)
        ...
        new_depth, new_height, new_width = healthy_downsampled.shape[2:]
        healthy_downsampled = healthy_downsampled.reshape(new_depth, new_height, new_width)
    else:
    ...
    if height_downsampled % 2 == 0:
        lower_half = healthy_downsampled[:, mid:, :]
    else:
        lower_half = healthy_downsampled[:, mid+1:, :]
    
    # Mirror the lower half for comparison (flip along height dimension, axis=1)
    lower_half_mirrored = torch.flip(lower_half, dims=[1])
    
    # Calculate the absolute difference between the mirrored lower half and the upper half
    difference = torch.abs(upper_half - lower_half_mirrored)
...
def normalize_intensities(wm_intensity, gm_intensity, csf_intensity, c):
    ...
    normalized_gm = torch.where(below_threshold, equal_proportion, gm_intensity / total)
    normalized_csf = torch.where(below_threshold, equal_proportion, csf_intensity / total)
    normalized_c = torch.where(below_threshold, equal_proportion, c / total)
...
def compute_strain_balance_tf(E, c, gamma, lambda_vals, mu_vals, domain):
    ...
    for t in range(0, c.shape[0]):
        ...
        tumor_force = gamma *torch.stack([grad_c_x, grad_c_y, grad_c_z], dim=0)  
    ...
    return torch.stack(residuals)  # Stack the residuals to form a tensor
...
def divergence_fd(tensor, dx, dy, dz):
    ...
    tensor_right = torch.roll(tensor, shifts=(-1,), dims=(-3,))
    tensor_up = torch.roll(tensor, shifts=(-1,), dims=(-2,))
    tensor_forward = torch.roll(tensor, shifts=(-1,), dims=(-1,))

    # Shift tensor in x, y, and z directions
    tensor_left = torch.roll(tensor, shifts=(1,), dims=(-3,))
    tensor_down = torch.roll(tensor, shifts=(1,), dims=(-2,))
    tensor_back = torch.roll(tensor, shifts=(1,), dims=(-1,))
...
def compute_stress(E, lambda_, mu):
    ...
    # 创建单位矩阵并增加维度
    I = torch.eye(3, dtype=dtype, device=DEVICE).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)  # size (3, 3, 1, 1, 1)
    I = I.expand_as(E)  # size (3, 3, nx, ny, nz)
...
def tumor_pde_loss(tx, ty, tz, dt, c, D, rho):
    ...
    dc_dt = (c - torch.roll(c, shifts=1, dims=0)) / dt

    # Reaction term (calculated at the mid-point in time)
    c_mid = 0.5 * (c + torch.roll(c, shifts=1, dims=0))
    reaction = torch.abs(c_mid) * (1 - c_mid) * rho

    # Calculate Δx, Δy, and Δz
    dx = torch.roll(tx, shifts=-1, dims=1) - tx
    dy = torch.roll(ty, shifts=-1, dims=2) - ty
    dz = torch.roll(tz, shifts=-1, dims=3) - tz

    # Calculate Δc in x, y, and z directions
    dc_x = torch.roll(c, shifts=-1, dims=1) - c
    dc_y = torch.roll(c, shifts=-1, dims=2) - c
    dc_z = torch.roll(c, shifts=-1, dims=3) - c

    # Diffusion terms
    diffusion_x = (D["D_minus_x"] * dc_x / dx - D["D_plus_x"] * torch.roll(dc_x / dx, shifts=1, dims=1)) / ((dx + torch.roll(dx, shifts=1, dims=1)) / 2)
    diffusion_y = (D["D_minus_y"] * dc_y / dy - D["D_plus_y"] * torch.roll(dc_y / dy, shifts=1, dims=2)) / ((dy + torch.roll(dy, shifts=1, dims=2)) / 2)
    diffusion_z = (D["D_minus_z"] * dc_z / dz - D["D_plus_z"] * torch.roll(dc_z / dz, shifts=1, dims=3)) / ((dz + torch.roll(dz, shifts=1, dims=3)) / 2)
    
    diffusion = diffusion_x + diffusion_y + diffusion_z
    diffusion = 0.5 * (diffusion + torch.roll(diffusion, shifts=1, dims=0))  # Diffusion terms at mid-point in time
...
def initialize_c(nx, ny, nz, x_center, y_center, z_center, radius):
    ...
    return torch.from_numpy(c_init).to(dtype=dtype)
...
def state_to_traj(domain, state, mod=torch):
...
def particles_to_field_3d(up, tx, ty, tz, domain):
    ...
    tx, ty, tz: trajectories of particles, shape (nx, ny, nz) as PyTorch tensors
    '''
    dx = domain.step('x')
    dy = domain.step('y')
    dz = domain.step('z')
    ...
    tix = torch.clamp(torch.floor(tx / dx - 0.5), 0, nx - 2)
    tiy = torch.clamp(torch.floor(ty / dy - 0.5), 0, ny - 2)
    tiz = torch.clamp(torch.floor(tz / dz - 0.5), 0, nz - 2)
    tix = tix.long()
    tiy = tiy.long()
    tiz = tiz.long()
    tixp = tix + 1
    ...
    sz1 = torch.clamp(tz / dz - 0.5 - tiz.float(), 0., 1.)
    sx0 = 1 - sx1
    sy0 = 1 - sy1
    sz0 = 1 - sz1

    # Flatten the particle values
    up_flat = up.reshape(-1)

    # Initialize the output field and weight tensors
    u = torch.zeros_like(tx, dtype=domain.dtype)
    w = torch.zeros_like(tx, dtype=domain.dtype)

    # Flatten all indices and weights for scatter operation
    indices_list = []
    weights_list = []
    
    # Generate all 8 combinations of weights and corresponding indices
    weight_combinations = [
        (tix, tiy, tiz, sx0 * sy0 * sz0),
        (tixp, tiy, tiz, sx1 * sy0 * sz0),
        (tix, tiyp, tiz, sx0 * sy1 * sz0),
    ...
    for jx, jy, jz, jw in weight_combinations:
        ...
        flat_weights = jw.reshape(-1)
        
        indices_list.append(flat_indices)
    ...
    all_up_weights = torch.cat([up_flat * w for w in weights_list], dim=0)

    # Use scatter_add to accumulate values
    u_flat = u.reshape(-1)
    w_flat = w.reshape(-1)
    
    # Convert 3D indices to 1D indices for scatter_add
    linear_indices = all_indices[:, 0] * (ny * nz) + all_indices[:, 1] * nz + all_indices[:, 2]
    linear_indices =  linear_indices.long()
    
    u_flat.scatter_add_(0, linear_indices, all_up_weights)
    w_flat.scatter_add_(0, linear_indices, all_weights)
...
def particles_to_field(up, tx, ty, tz, domain): # be very careful
    ...
    tiz = torch.clamp(torch.floor(tz / dz - 0.5), 0, nz - 2).long()
    tixp = tix + 1
    tiyp = tiy + 1
    tizp = tiz + 1
...
# def particles_to_field_tf(up, tx, ty, tz, domain): # maybe not use!
#     '''
#     up: values carried by particles, shape (nx, ny, nz)
...
def report_func(problem, state, epoch, cbinfo):
    ...
    printlog('dice={}'.format( [ten2arr( edema_dice ), ten2arr( core_dice )]))
...
def process_and_save_solution(problem, state, wm_data, gm_data, csf_data, epoch):
    ...
    c_euler = ten2arr(lagrange_to_euler_all_slices(c_lagrange, tx, ty, tz, domain))
...
def plot(problem, state, epoch, frame, cbinfo=None):
    ...
    th_down = torch.clamp(domain.field(state, 'coeff')[5], min=0.20, max=0.35)
    # Clip th_up values
    # Ensuring th_up is no less than 0.50 and no more than 0.85
    # th_up = tf.clip_by_value(domain.field(state, 'coeff')[6], clip_value_min=0.50, clip_value_max=0.85)
    th_up = torch.clamp(domain.field(state, 'coeff')[6], min=0.50, max=0.85)
...
def process_data(args, matter_th):
    ...
    tumor_core_mask = get_core_mask(segm_data, mod=torch)  # Using numpy here as we're working with numpy arrays
    ...
    print("\n----------------------------------------------------------------------------")
    outside_matter_mask = mod.array2tensor(outside_matter_mask, DTYPE).to(DEVICE)
    outside_skull_mask = mod.array2tensor(outside_skull_mask, DTYPE).to(DEVICE)
    wm_data = mod.array2tensor(wm_data, DTYPE).to(DEVICE)
    print(f"wm_data: dtype={wm_data.dtype}, shape={wm_data.shape}, device={wm_data.device}")

    gm_data = mod.array2tensor(gm_data, DTYPE).to(DEVICE)
    print(f"gm_data: dtype={gm_data.dtype}, shape={gm_data.shape}, device={gm_data.device}")

    csf_data = mod.array2tensor(csf_data, DTYPE).to(DEVICE)
    print(f"csf_data: dtype={csf_data.dtype}, shape={csf_data.shape}, device={csf_data.device}")

    # segm_data = get_data(args.segmfile).astype(int)
    segm_data = mod.array2tensor(segm_data, torch.int).to(DEVICE)
    print(f"segm_data: dtype={segm_data.dtype}, shape={segm_data.shape}, device={segm_data.device}")

    pet_data = mod.array2tensor(pet_data, DTYPE).to(DEVICE)
    print(f"pet_data: dtype={pet_data.dtype}, shape={pet_data.shape}, device={pet_data.device}")
    print("----------------------------------------------------------------------------\n")
...
def make_problem(args):
    ...
    'gm': ten2arr(gm_data),      # Grey matter data
    'wm': ten2arr(wm_data),      # White matter data
    ...
    result = sol.solve()         # 只会用到一次前向模拟器，作为初始值猜测
    FK_ts = result['time_series']
    print("Fisher-KPP time-spatial data [Nt, Nx, Ny, Nz]: ", FK_ts.shape)
