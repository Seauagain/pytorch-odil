...
Value = Union[float, str, np.float32, np.float64, np.ndarray, None]


class History:

    def __init__(self, csvpath: Union[str, None] = None, warmup: int = 0):
        """
        warmup: `int`
    ...
    def get(self, key: str, default: Union [ Value, None ] = None) -> Value:        
    ...
    def write(self, nocommit: bool = False) -> None:
        ...
        if self.csvkeys is not None:
            if len(self.data.keys()) != len(self.csvkeys):
                ...
                raise RuntimeError("Unexpected keys in history: {:}".format(newkeys))
        if self.csvcount == 0:
            # Write header.
