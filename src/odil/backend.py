...
class ModTorch(ModBase):
    ...
    def __init__(self, mod=None, modsp=None):
        ...
        self.modsp = modsp
        self.floor = mod.floor
        self.arctan2 = mod.atan2
        self.concatenate = mod.cat
        self.constant = mod.tensor
        self.is_tensor = mod.is_tensor
        self.clip = mod.clamp 
        self.copy = lambda x: x.clone()
        # self.gather_nd = mod.gather_nd
        # self.batch_to_space = mod.batch_to_space
        self.conv_transpose = mod.nn.functional.conv_transpose2d
        self.random = mod ## to be implemented
        ...
        def convolution(inputs, filters, strides, padding):
            """
            inputs: n-dimensional input array
            filters: n-dimiensional kernel
            strides: `int` or sequence of n integers
            """
            # add batch_dim and channel_dim
            inputs = inputs.unsqueeze(0).unsqueeze(0)   # [1, 1, D1, ..., Dn]
            filters = filters.unsqueeze(0).unsqueeze(0)  # [1, 1, K1, ..., Kn]
            res = mod.nn.functional.conv2d(input, filters, stride=strides, padding=padding)
        ...
        self.random.set_seed = set_seed
        self.random.uniform = lambda low=0.0, high=1.0, size=None: \
            (low + (high - low) * torch.rand(size))
        self.random.normal = lambda mean=0.0, std=1.0, size=None: \
            (mean + std * torch.randn(size))
        ...
        if modsp is not None:
            ...
            # override: if scipy sparse, return directly
            self.numpy = lambda x: x if isinstance(x, modsp.csr_matrix) else tmp(x)
...
#     def __init__(self, mod=None, modsp=None):
#         super().__init__(mod)
#         self.batch_to_space = mod.batch_to_space # no use
#         self.cast = mod.cast # type convert
#         self.numpy = lambda x: x.numpy()
#         self.native = lambda x: x
#         self.spnative = lambda x: x.numpy() if hasattr(x, "numpy") else x
